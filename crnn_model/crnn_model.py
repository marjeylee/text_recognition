...
class ShadowNet(cnn_basenet.CNNBaseModel):
    ...
    def __conv_stage_no_pooling(self, inputdata, out_dims, name=None):
        """
        Traditional conv stage in VGG format
        :param inputdata:
        :param out_dims:
        ...
        conv = self.conv2d(inputdata=inputdata, out_channel=out_dims, kernel_size=3, stride=1, use_bias=False,
                           name=name)
        relu = self.relu(inputdata=conv)
        max_pool = self.maxpooling(inputdata=relu, kernel_size=2, stride=[2, 1])
        return max_pool
    ...
    def __feature_sequence_extraction(self, inputdata):
        ...
        :return:
        """
        conv1 = self.__conv_stage_no_pooling(inputdata=inputdata, out_dims=64, name='conv1')  # batch*16*50*64
        ...
        conv5 = self.conv2d(inputdata=relu4, out_channel=512, kernel_size=3, stride=1, use_bias=False,
                            name='conv5')  # batch*4*25*512
        if self.phase.lower() == 'train':
            bn5 = self.layerbn(inputdata=conv5, is_training=True)
        else:
            bn5 = self.layerbn(inputdata=conv5, is_training=False)  # batch*4*25*512
        relu5 = self.relu(bn5)  # batch*4*25*512

        conv6 = self.conv2d(inputdata=relu5, out_channel=512, kernel_size=3, stride=1, use_bias=False,
                            name='conv6')  # batch*4*25*512
        if self.phase.lower() == 'train':
            bn6 = self.layerbn(inputdata=conv6, is_training=True)
        else:
            bn6 = self.layerbn(inputdata=conv6, is_training=False)  # batch*4*25*512
        relu6 = self.relu(bn6)  # batch*4*25*512

        max_pool6 = self.maxpooling(inputdata=relu6, kernel_size=[2, 1], stride=[2, 1])  # batch*2*25*512
        conv7 = self.conv2d(inputdata=max_pool6, out_channel=512, kernel_size=3, stride=[2, 1], use_bias=False,
                            name='conv7')  # batch*1*25*512
        relu7 = self.relu(conv7)  # batch*1*25*512
        conv8 = self.conv2d(inputdata=relu7, out_channel=512, kernel_size=3, stride=[2, 1], use_bias=False,
                            name='conv8')  # batch*1*25*512
        if self.phase.lower() == 'train':
            conv8 = self.layerbn(inputdata=conv8, is_training=True)
        else:
            conv8 = self.layerbn(inputdata=conv8, is_training=False)  # batch*4*25*512
